...
// Most common library is go-jose - but it's pretty heavy and complex.


// JWTHead is the first part of the token.
type JWTHead struct {
...
func GetToken(ctx context.Context, priv crypto.PrivateKey, sub, aud, iss string) (string, error) {
	jwt := &JWT{
		Aud: []string{aud},
	...
	return jwt.Sign(priv), nil
}


type CATokenSource struct {
	Sub string
	CA  crypto.PrivateKey
	Iss string
}
