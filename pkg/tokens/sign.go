...
func SignHash(data []byte, p crypto.PrivateKey) []byte {
	...
	if ec, ok := p.(*ecdsa.PrivateKey); ok {
		...
		signer := SignerFromKey(p)
		sig1, _ := signer.Sign(rand.Reader, data, crypto.SHA256)
		return sig1
...
func SignTo(pk crypto.PrivateKey, data []byte, sig []byte) {
	hasher := crypto.SHA256.New()
	hasher.Write(data) //[0:64]) // only public key, for debug
	hash := hasher.Sum(nil)
	...
	if ec, ok := pk.(*ecdsa.PrivateKey); ok {
		...
		copy(sig, r.Bytes())
		copy(sig[32:], s.Bytes())
	} else if ed, ok := pk.(ed25519.PrivateKey); ok {
		sig1, _ := ed.Sign(rand.Reader, hash, nil)
		copy(sig, sig1)
	} else {
		signer := SignerFromKey(pk)
